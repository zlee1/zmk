...
static const int CENTRAL = IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL);
...
static void zmk_rgb_underglow_effect_random(void) {
    for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
        if (state.animation_step % (11 - (state.animation_speed * 2)) == 0) {
            struct zmk_led_hsb hsb = state.color;
            hsb.h = rand() % HUE_MAX;
    ...
    if(CENTRAL){
        for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
            ...
            if(animation_step_central[i] == 0){
                ...
                hsb.b = (float)BRT_MAX-((float)BRT_MAX*(1.0-((float)(animation_step_central[i]-peak_step)/(float)(end_step-peak_step))));
            }
...
static int key_press_event_listener(const zmk_event_t *eh) {
    ...
    if(ev && state.current_effect == UNDERGLOW_EFFECT_REACTIVE) { 
        ...
        // leds
        // 18, 17, 12, 11, 4, 3,   3, 4, 11, 12, 17, 18,
        // 19, 16, 13, 10, 5, 2,   2, 5, 10, 13, 16, 19,
        // 20, 15, 14,  9, 6, 1,   1, 6,  9, 14, 15, 20,
        //              8, 7, 0,   0, 7,  8
